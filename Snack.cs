    ...
    public partial class Snack : Form
    {
        ...
        private void FetchSnackData()
        {
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                ...
                {
                    conn.Open();
                    string query = "SELECT MenuItemId, ItemName, Price, Description, ImagePath FROM MenuItems WHERE CategoryId = (SELECT CategoryId FROM Categories WHERE CategoryName = 'Snacks')";
                    ...
                    while (reader.Read())
                    {
                        ...
                        snacks.Add(reader.GetString("ItemName"));
        ...
        private void SetUpSnackGrid()
        {
            ...
            for (int rowIndex = 0; rowIndex < snackGrid.RowCount - 1; rowIndex++)
            {
                for (int columnIndex = 0; columnIndex < snackGrid.ColumnCount; columnIndex++)
                {
                    ...
                    {
                        ImageLocation = imagePaths[snackIndex], // Use the image path from the database
                    ...
                    int currentSnackIndex = snackIndex; // Capture the index
                    productBox.Click += (sender, e) => productBox_selected(sender, e, currentSnackIndex);
        ...
        private void productBox_selected(object sender, EventArgs e, int snackIndex)
        {
            ...
            {
                ImageLocation = imagePaths[snackIndex],  // Set the image based on the selected index
            ...
            addToOrder.Click += (s, e) =>
            {
                ...
                int selectedMenuItemId = menuItemIds[snackIndex]; // Assuming you have an array of ice creams and their IDs.
                decimal selectedPrice = prices[snackIndex];
                ...
                if (Form1.orderList.ContainsKey(snacks[snackIndex]))
                {
                    Form1.orderList[snacks[snackIndex]] = (prices[snackIndex], amountToOrder);
                }
                else
                {
                    Form1.orderList.Add(snacks[snackIndex], (prices[snackIndex], amountToOrder));
                }
            ...
            if (Form1.orderList.ContainsKey(snacks[snackIndex]))
            {
                productPriceDisplay.Text = $"{(Form1.orderList[snacks[snackIndex]].Price) * (Form1.orderList[snacks[snackIndex]].AmountToOrder)}";
                amountToOrder = Form1.orderList[snacks[snackIndex]].AmountToOrder;
                localCounterLabel.Text = Convert.ToString(amountToOrder);
            }
            else
            {
                productPriceDisplay.Text = $"{prices[snackIndex]}";
            }
